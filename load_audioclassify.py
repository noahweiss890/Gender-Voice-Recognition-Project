...
def featurize(wavfile):
    ...
    features_original = np.append(featurize2(wavfile),audio_time_features(wavfile))
    features = np.append(features_original,embedding.tolist()).reshape(1,-1)
    scaled_features = scaler_reload.transform(features)
    # pca_features = np.append(pca_reload.transform(scaled_features[0][:208].reshape(1,-1)), scaled_features[0][208:])
    # pca_features = np.append(scaled_features[0][:-512], pca_reload.transform(scaled_features[0][-512:].reshape(1,-1)))
    pca_features = pca_reload.transform(scaled_features)
    return pca_features
...
scaler_reload = pickle.load(open(f"data/scaler.sav",'rb'))
pca_reload = pickle.load(open(f"data/pca.pkl",'rb'))
