...
#     except:
    ...
    if maxacc==c1:
        ...
        stop_now = time.time()
        execution = stop_now - start_now
        print("execution time for model:", execution)
    elif maxacc==c5:
    ...
    # find precision and recall for the best classifier
    lb = LabelBinarizer()
    y_train = np.array([number[0] for number in lb.fit_transform(labels_test_set2)])
    precision = cross_val_score(classifier, test_set2, y_train, cv=5, scoring='precision')
    recall = cross_val_score(classifier, test_set2, y_train, cv=5, scoring='recall')
...
# load classifier for the x-vector embedding
            ...
            try:
                ...
                if file[-4:]=='.wav' not in dirlist and os.path.getsize(file)>500:
                    try:
                        ...
                        #obtain features 
                        if bool(random.getrandbits(1)): # flip virtual coin
                            wavfile = f"../{name}_aug/{wavfile}"
                        ...
                        features=np.append(features,embedding.tolist())
                        #append to list 
...
def prepare_data_for_pca_on_208(all_data):
    classes_all=list(all_data)
    all_features = list()
    all_labels = list()
    for i in range(len(classes_all)):
        for j in range(len(all_data[classes_all[i]])):
            feature=all_data[classes_all[i]][j]
            all_features.append(feature)
            all_labels.append(classes_all[i])
    scaler = StandardScaler()
    all_data_rescaled = scaler.fit_transform(all_features)
    the_208 = list()
    the_rest = list()
    for i in range(len(all_data_rescaled)):
        the_208.append(all_data_rescaled[i][:208])
        the_rest.append(all_data_rescaled[i][208:])
    pca_208 = PCA().fit(the_208)
    chosen_n_components_208 = find_optimal_n_components(pca_208.explained_variance_ratio_)[1]
    print("chosen_n_components_208: ", chosen_n_components_208)
    pca_proper_208 = PCA(n_components=chosen_n_components_208)
    pca_proper_208.fit(the_208)
    reduced_208 = pca_proper_208.transform(the_208)
    pk.dump(pca_proper_208, open("pca.pkl", "wb"))
    pk.dump(scaler, open("scaler.sav", "wb"))
    result = list()
    for i in range(len(all_data_rescaled)):
        result.append(np.concatenate((reduced_208[i], the_rest[i])))
    return result, all_labels

def prepare_data_for_pca_on_512(all_data):
    classes_all=list(all_data)
    all_features = list()
    all_labels = list()
    for i in range(len(classes_all)):
    ...
    the_512 = list()
    for i in range(len(all_data_rescaled)):
        the_rest.append(all_data_rescaled[i][:-512])
        the_512.append(all_data_rescaled[i][-512:])
    pca_512 = PCA().fit(the_512)
    chosen_n_components_512 = find_optimal_n_components(pca_512.explained_variance_ratio_)[1]
    print("chosen_n_components_512: ", chosen_n_components_512)
    pca_proper_512 = PCA(n_components=chosen_n_components_512)
    pca_proper_512.fit(the_512)
    reduced_512 = pca_proper_512.transform(the_512)
    pk.dump(pca_proper_512, open("pca.pkl", "wb"))
    pk.dump(scaler, open("scaler.sav", "wb"))
    result = list()
    for i in range(len(all_data_rescaled)):
        result.append(np.concatenate((the_rest[i], reduced_512[i])))
    return result, all_labels

def prepare_data_for_pca_on_720(all_data):
    classes_all=list(all_data)
    all_features = list()
    all_labels = list()
    for i in range(len(classes_all)):
    ...
    scaler = StandardScaler()
    all_data_rescaled = scaler.fit_transform(all_features)
    pca_720 = PCA().fit(all_data_rescaled)
    chosen_n_components_720 = find_optimal_n_components(pca_720.explained_variance_ratio_)[1]
    print("chosen_n_components_720: ", chosen_n_components_720)
    pca_proper_720 = PCA(n_components=chosen_n_components_720)
    pca_proper_720.fit(all_data_rescaled)
    reduced_720 = pca_proper_720.transform(all_data_rescaled)
    pk.dump(pca_proper_720, open("pca.pkl", "wb"))
    pk.dump(scaler, open("scaler.sav", "wb"))
    result = reduced_720.tolist()
    return result, all_labels

def prepare_data_for_pca_on_none(all_data):
    classes_all=list(all_data)
    all_features = list()
    all_labels = list()
    for i in range(len(classes_all)):
...
def find_optimal_n_components(explained_variance_ratio):
    derivatives = []
...
# load the dataset
all_data = json.load(open("males_females_audio.json"))

# result, all_labels = prepare_data_for_pca_on_none(all_data) # no pca at all
# result, all_labels = prepare_data_for_pca_on_208(all_data) # pca on 208 features
# result, all_labels = prepare_data_for_pca_on_512(all_data) # pca on 512 features
result, all_labels = prepare_data_for_pca_on_720(all_data) # pca on all 720 features

train_set, test_set, train_labels, test_labels = train_test_split(result,
                                                                  all_labels,
...
min_num=len(all_data[list(all_data)[0]])
[audio_model, audio_acc, audio_summary, data]=optimizemodel_sc(train_set,train_labels,test_set,test_labels,modelname,list(all_data),testing_set,min_num,selectedfeature,training_data)
